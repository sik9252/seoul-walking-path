...
## 9) 기록 필터 분류 규칙 (산책/트레킹/등산)

- 목적: `나의 기록` 필터(`산책`, `트레킹`, `등산`)를 데이터 소스가 달라도 일관되게 적용
- 서울둘레길(OA-22438) 기본값
  - 기본 분류: `트레킹`
  - 예외 상향: `LV_KORN=상급` + 장거리(예: 8km 이상)면 `등산`
  - 예외 하향: `LV_KORN=초급` + 단거리/완만 구간은 `산책`
- 일반 산책로 데이터 확장 시
  - 소스별 원본 필드를 `distanceKm`, `durationMin`, `difficulty`, `elevationGain(가능 시)`로 정규화
  - 동일 규칙으로 카테고리 자동 판정
  - 애매한 케이스는 `트레킹` 기본값 적용 후 수동 태그 보정 가능하도록 설계
