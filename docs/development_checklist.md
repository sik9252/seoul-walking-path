...
- [x] 서울둘레길 코스의 기본 분류 규칙 정의(산책/트레킹/등산)
- [ ] 기록 저장 시 코스 분류값(`walkType`) 저장 필드 추가
- [ ] 나의 기록 필터(산책/트레킹/등산)를 `walkType` 기반으로 연결
- [ ] 일반 산책로 데이터 유입 시 공통 분류 규칙 적용 검증
...
- Android 배포 준비: `eas.json` 추가, 앱 ID(`com.beank.seoulwalkingpath`)와 버전 전략, 내부테스트 배포 가이드 초안 문서화.
...
- 기록 필터 분류 기준 합의: 서울둘레길 데이터는 기본 `트레킹`, 규칙 기반으로 `산책/등산` 예외 분류.
